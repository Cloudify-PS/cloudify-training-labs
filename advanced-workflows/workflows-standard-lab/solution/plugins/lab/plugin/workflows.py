...
def check_wind_speed(ctx, city_name,
                     scalable_entity_name, max_wind_speed, **_):
    '''
    ...
    # Scale up if winds are OK, scale down if they're above max
    delta = 1 if (speed < max_wind_speed) else -1
    ctx.logger.info('Scalable entity instance count (pre): %s'
                    % get_scale_group_properties(
                        ctx, scalable_entity_name)['current_instances'])
    # Scale based on a scale delta
    scale_if_needed(ctx, scalable_entity_name, delta)
    ctx.logger.info('Scalable entity instance count (post): %s'
                    % get_scale_group_properties(
                        ctx, scalable_entity_name)['current_instances'])
